...
# Estimate the logical error rate with post-selection
def estimate_logical_cnot_error_zero(p,num_shots):
    global detector_now, N
    ...
    for q in range(Q):
        ...
        append_noisy_wait(circuit,range(0,N), p,(2*T+6)*above_qubit)
        append_noisy_wait(circuit,range(2*N,3*N), p,(2*T+6)*above_qubit)
    ...
    err = sum([count_errors(x,list_detector_m,list_detector_X,list_detector_Z,Q)[0] for x in sample])
    logical_error_zero = err/(num*Q)
    variance_zero = err/(num*Q)**2

    return logical_error_zero, variance_zero

# Estimate the logical error rate with one verification failing
def estimate_logical_cnot_error_one(p, num_shots):
    global detector_now, N
    ...
    for error_location in range(4*Q):
        error_q = error_location//4
        error_which = (error_location%4)//2
        error_which2 = error_location%2
        ...
        for q in range(Q):
            ...
            if q==error_q:
                if error_which==0:
                    [detector_0prep, detector_Z, detector_X] = append_noisy_ec(circuit,0,4*N,5*N, p, error_which2)
                    list_detector_0prep.extend(detector_0prep)
                    list_detector_X.append(detector_X)
                    list_detector_Z.append(detector_Z)
                    append_noisy_wait(circuit,range(2*N,3*N), p,T+2)
                ...
                if error_which==1:
                    ...
                    [detector_0prep, detector_Z, detector_X] = append_noisy_ec(circuit,2*N,6*N,7*N, p, error_which2)
                    list_detector_0prep.extend(detector_0prep)
                    list_detector_X.append(detector_X)
                    list_detector_Z.append(detector_Z)
                    append_noisy_wait(circuit,range(0,N), p,T+2)
            else:
                [detector_0prep, detector_Z, detector_X] = append_noisy_ec(circuit,0,4*N,5*N, p, -1)
                list_detector_0prep.extend(detector_0prep)
                list_detector_X.append(detector_X)
        ...
        logical_error_one += err/(num*Q)
        variance_one += err/(num*Q)**2

    return logical_error_one, variance_one

def estimate_logical_cnot_error(p, num_shots):
    prob_0prep, prob_0prep_variance = estimate_prob_0prep(p,int(num_shots/100))
    logical_error_zero, variance_zero = estimate_logical_cnot_error_zero(p, num_shots)
    logical_error_one, variance_one = estimate_logical_cnot_error_one(p, int(num_shots/100))
    logical_error = logical_error_zero+(1.0-prob_0prep)*logical_error_one
    variance = variance_zero+(1.0-prob_0prep)**2*variance_one+logical_error_one**2*prob_0prep_variance
...
if __name__ == '__main__':
    ...
    r = int(sys.argv[6])
    above_qubit = 2**(int(sys.argv[7]))-1
    ...
    if r==3:
        ...
        # For r>3, generate latin rectangle randomly
        random.seed(10)

        # Initialization
        latin_rectangle = np.zeros([N, r])
        for i in range(r):
            for n in range(N):
                if check_matrix[i,n]==1 and 2**i-1 != n:
                    latin_rectangle[n,i]=-1

        # Putting numbers greedy
        for num in range(1,2**(r-1)):
            occupied=np.array([])
            for i in range(r):
                for n in random.sample(list(range(N)), N):
                    if latin_rectangle[n,i]==-1 and not(np.any(occupied==n)):
                        latin_rectangle[n,i]=int(num)
                        occupied = np.append(occupied,n)
                        break
        T=2**(r-1)-1
